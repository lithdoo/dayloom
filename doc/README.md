# Dayloom 文档

**状态**：Published
**最后核对**：2026-08-24

从 [快速开始](/guide/GETTING_STARTED) 运行正式 TUI。应用和基础设施的稳定边界分别由以下契约定义：

- [Core Runtime V1](/contracts/CORE_RUNTIME_V1)
- [World Profile V1](/contracts/WORLD_PROFILE_V1)
- [Archive 格式](/reference/ARCHIVE_FORMAT)

常用资料：

- [TUI 指南](/guide/TUI)
- [World 生命周期](/guide/WORLD_LIFECYCLE)
- [Commands 与状态](/reference/COMMANDS)
- [配置](/reference/CONFIGURATION)
- [环境变量](/reference/ENVIRONMENT_VARIABLES)
- [故障排查](/guide/TROUBLESHOOTING)
- [迁移到正式 Core](/guide/MIGRATION)
- [1.0.0-beta.1 发布说明](/reference/RELEASE_NOTES_1.0.0-beta.1)
- [测试概览](/testing/OVERVIEW)

历史设计与已实施计划位于 `doc/archive/`，不构成当前产品契约。


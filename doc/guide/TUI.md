...
Hub 是稳定边界页，Session 是对话页。Core event 只同步事实；调用 Core API 的 Promise/CoreResult 与最终 `getState()` 决定页面切换，避免 transient event 提前切页。
...
- Business action：`init`、`daily`、`revise`、`play`、`settle`、`abandon-day`，只展示 Core capabilities 当前允许的项。
- Local action：`status`、`help`、`quit`，不调用 Core。
...
| `/exit` / `/cancel` | 执行 Core `cancel`；running 时中断 AI，并由 cancel 结果决定是否回 Hub |
...
- terminal failure 保留 failed transcript；此时 `/exit`、`/cancel` 只关闭展示，不调用 Core，也不把最近结果改写为取消。

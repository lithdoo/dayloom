...
本文冻结 Core MVP 的行为、边界、public contract 与实施顺序。
...
现有交互方式只提供 capability reference。Core 的 application API 必须自然表达：
...
Core 不复制第二套 Archive Protocol DTO，也不 deep-import `src/*` / `dist/*`。
...
Core **不得直接创建、命名、修改 Promptpile message / receipt artifacts**。
...
Core 不重新实现 React FSM，不导入 React runtime internals。

Core owns React 的**调用策略**：Conversation topology、max-step、Core system prompts、是否允许 tools/hook，以及如何消费公开 Agent Event Protocol。
...
`packages/core/package.json` direct dependencies：
...
```text
promptpile
= Core 使用 public conversation append-user CLI

promptpile-react
= Core 使用 packaged executable + packaged Agent Event v1 schema

@iarna/toml
= Core 只检查/派生自己拥有的 React orchestration config；不解释 provider 语义
...
原因：React orchestration 属于 Core ownership，不应由 consumer 提供完整 React config。

`llmConfigPath` 只提供 Promptpile 的 LLM/provider/profile 数据。Core 不解释 model、API key、base URL、temperature、extra body 等 provider 语义；这些仍由 Promptpile / Promptpile React 校验。
...
Core 在 initialization 时用 `@iarna/toml` parse `llmConfigPath`，只做**ownership guard**。
...
这些字段属于 Core 已冻结的 runtime/orchestration policy。

其它 provider/profile 数据由 Core 原样保留，不自行解释。特别是：
...
Core 不把 caller config 原样当 React config 使用。
...
3. 增加 Core-owned `[promptpile-react]`；
4. 用 `@iarna/toml` stringify；
5. 两个 derived config 只在 `final_prompt` 上不同。

Send config 的 Core-owned table 等价于：

```toml
[promptpile-react]
max_step = 1
thought_prompt = "<absolute core thought prompt>"
final_prompt = "<absolute core send-final prompt>"
```
...
max_step = 1
thought_prompt = "<absolute core thought prompt>"
final_prompt = "<absolute core submit-final prompt>"
```

Core 仍在 CLI 显式传 `--max-step 1`，使 runtime policy 不依赖 config merge 偶然行为。
...
Core 不在 Archive root 发明 private layout。

每个 Core instance 创建 OS temporary runtime root：

```text
<os.tmpdir()>/dayloom-core-<mkdtemp>/
└── sessions/
    └── <sessionId>/
        ├── context/
        ├── conversation/
...
contains exactly one Core-appended context user artifact
...
Core 不维护第二份 `messages[]` authority。
...
Core 从 resolved `promptpile-react/package.json` 所在目录定位该 schema，读取 JSON，并用 Ajv 2020 compile 一次。

Core **不得复制或手写另一份 Agent Event v1 JSON schema**。

Schema 允许的 forward-compatible optional fields 必须继续允许；Core 不额外做 top-level exact-key rejection。

找不到 bin/package metadata/schema，或 schema 无法 compile → initialization `INTERNAL_ERROR`。

---

## 15. Core-owned React prompts 冻结
...
这些文件是 Core private runtime prompt，不是 Promptpile Conversation artifact。

---

## 16. Conversation append contract 冻结

Core 不使用 `promptpile-react --input`。
...
Core 不支持并发，因此 MVP 不使用 Promptpile fingerprint / expected-next-index OCC 参数。
...
Core 生成文本，但通过 Promptpile CLI append；不直接制造 artifact。
...
→ Core stream invariants
```

Core 只验证跨 event 的 transport/terminal invariants，不重新验证 React phase FSM。
...
Core 不检查 Thought/Observe/Check 的内部 phase 顺序；那属于 Promptpile React ownership。
...
真正的 JSON schema / authority 来自 Core-owned `final-submit.md` system prompt，不依赖这条 user-role marker。
...
Core 从 validated plan + submission 重建：
...
```text
core-rejects-caller-promptpile-react-table
core-rejects-caller-conversation-or-tool-or-hook-config
core-preserves-llm-profile-data-in-derived-config
core-derived-config-owns-max-step-and-prompt-paths
core-resolves-packaged-promptpile-binaries
core-resolves-and-compiles-packaged-agent-event-schema
core-does-not-copy-agent-event-schema

core-init-validates-current-graph
core-init-rejects-invalid-current-graph
core-init-rejects-malformed-planned-context
core-play-only-available-from-planned-world-with-day

core-start-play-does-not-publish-world
core-start-play-creates-immutable-context-and-writable-conversation
core-context-is-appended-via-promptpile-cli
core-never-writes-promptpile-message-files-directly
core-user-turns-never-append-to-context-layer
core-two-turns-share-one-writable-conversation

core-react-explicitly-uses-context-plus-output-conversation
core-react-max-step-is-one
core-react-does-not-enable-application-tools-or-hooks
core-rejects-malformed-agent-event
core-rejects-agent-event-schema-violation
core-allows-schema-forward-compatible-optional-fields
core-rejects-sequence-gap-or-session-id-change
core-rejects-final-delta-content-mismatch
core-send-streams-only-final-deltas
core-submit-does-not-expose-json-output

core-parses-valid-play-submission-v1
core-rejects-unknown-submission-fields
core-validates-submission-against-pinned-plan
core-does-not-trust-model-day-or-beat-intent
core-play-json-has-frozen-bytes
core-summary-has-single-trailing-lf
core-rejects-play-history-overwrite

core-builds-candidate-with-archive-protocol-api
core-conflicting-pinned-base-fails-closed
core-does-not-wait-for-publish-lock
core-validates-prepared-target-relations
core-current-replacement-is-final-visibility-step
core-submit-publishes-awaiting-settle-exactly-once
core-previsibility-failure-leaves-current-unchanged
core-postvisibility-diagnostic-failure-does-not-report-false-rollback

core-rejects-second-mutation-with-busy
core-cancel-ready-session-leaves-world-unchanged
core-rejects-cancel-while-running
core-dispose-is-idempotent-and-does-not-publish
core-architecture-guard-rejects-legacy-core-tui-and-deep-imports
```

Acceptance tests 只验证 `@dayloom/core` public contract 与 infrastructure boundaries，不加入任何 TUI component/driver/adapter compatibility test。

---

## 30. Definition of Done

Core MVP 同时满足以下条件才完成：

1. `packages/core` 不依赖 legacy Core 或 presentation package；
2. 只读取/发布 Archive V2；
...
5. React orchestration config 与 Dayloom system prompts 由 Core runtime 生成；
6. Agent Event validation 使用 `promptpile-react` 随包 normative schema，不复制协议 schema；
7. Core 不重新实现 React FSM；
...
12. Core 从不直接制造 Promptpile message / receipt artifact；
13. 普通 Final delta 可公开，submission JSON Final 不公开为 presentation output；
14. Core 不支持并发，第二个 mutation 立即 `BUSY`；
...
18. persisted play/summary bytes 由 Core 从 validated values 重建；
...
26. current Conversation topology 可让未来 compression 只包裹 writable `conversation/`，无需重构 Core；
...
Core owns Dayloom business legality, React invocation policy, Session lifecycle and World publication.
...
不为旧 Core 保留兼容形状。
不为现有 TUI 设计专用接口。
不让 caller config 穿透 Core ownership。
...
→ Core-controlled React invocation

...
活跃文档只描述当前 `@dayloom/core` 和 `@dayloom/tui`。已弃用实现、未完成的迁移和尚未发布的应用入口不得出现在首页、导航或活跃参考中。
...
| `archive/plans/` | 已完成的实施计划，不上站 |
| `redirects/` | 旧路径兼容，不进导航 |

`doc/` 根目录只允许 `index.md`、`README.md`、`CONVENTIONS.md` 和站点资源。

## 3. 文档元数据

除首页和 archive 外，活跃文档文首应包含：
...
- 设计意图必须与现行实现分开；已完成的阶段计划进 archive。

## 5. 链接

- 站内链接使用 VitePress 根路径，例如 `/reference/COMMANDS`。
- 指向源码、测试、TODO 或 archive 时使用 `https://github.com/lithdoo/dayloom/...` 绝对链接。
- 活跃页不得使用 `../../packages/...` 这类离开 `doc/` 的相对链接。
- rename 时在 VitePress rewrites 或 `redirects/` 中保留旧路径。

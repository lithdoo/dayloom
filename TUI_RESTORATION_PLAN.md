...
```text
从 Git 历史完整恢复迁移到 Core2 之前的 @dayloom/tui 产品能力，
...
`c0d1b5f38c1d67cec62730ccd263c4429a490de1` 是将 `packages/tui` 从 `@dayloom/core` 改接到 `@dayloom/core2` 的迁移提交。
...
- `packages/tui/src/` 与 `packages/tui/test/` 都是完整的 pre-Core2 实现。

恢复时以 Git tree 为事实来源，不以当前 Core2 TUI 代码作为重构参考。
...
git revert 整段 Core2 工作
```

因为 Core2、Archive Protocol、Conversation Compression 等后续工作需要保留。
...
同时删除只在 Core2 迁移后新增、且 baseline 中不存在的 TUI 文件，例如：
...
并移除 `@dayloom/core2` 作为 TUI dependency。

### 4.2 `package-lock.json` — 只做 dependency reconciliation

禁止从 `cd10fb...` 整体覆盖当前 lockfile，因为这会丢失后续 Core2 / compression dependencies。
...
4. 确认 `@dayloom/core2` 自身及其新 dependencies 仍保留。
...
这里不做整个目录的机械 rollback，而是恢复 pre-Core2 **标准 TUI 使用路径**：
...
Core2 conversation compression implementation
promptpile-compress integration
Core2 独立 CI matrix
```

Core2 作为独立新实现继续存在，但在达到完整 functional parity 之前：
...
- 将 `TUI_CORE2_ADAPTATION_DRAFT.md` 标记为 `Superseded / Functional parity regression`，禁止再作为实施依据；
- `EXAMPLES_OPTIMIZATION_DRAFT.md` 中任何建立在“Core2 Play-only 已完整替代 Core”的结论都必须标记失效。
...
- `@dayloom/tui` 不 import `@dayloom/core2`；
...
2. TUI 重新依赖仍存在的 `@dayloom/core`，不依赖 Core2。
...
15. Core2、compression、Archive Protocol 后续成果没有被回滚。
16. `TUI_CORE2_ADAPTATION_DRAFT.md` 不再被视为有效完成标准。
17. 在完整恢复签收前，不开始第二次 Core2 TUI migration。
...
之后如果仍要让 Core2 最终替代 Core，必须先单独完成 functional parity：
...
并以完整 TUI 作为 parity consumer 验收；不能再次通过删减 TUI 功能来适配 Core2。

...
export interface CommandBoundaryV1 { command: string; argsPrefix: readonly string[] }
export interface PromptpileBoundariesV1 { promptpileBin: string; reactBin: string; promptpileMcpBin: string; filesystemMcp: CommandBoundaryV1 }
...
function commandBoundaryV1(metadata: Record<string, unknown>, name: string, root: string): CommandBoundaryV1 {
  ...
  return /\.[cm]?js$/i.test(target) ? Object.freeze({ command: process.execPath, argsPrefix: Object.freeze([target]) }) : Object.freeze({ command: target, argsPrefix: Object.freeze([]) });
}
export async function resolvePromptpileBoundariesV1(): Promise<PromptpileBoundariesV1> {
  const [promptpile, react, mcp, filesystem] = await Promise.all([
    packageRootV1('promptpile'), packageRootV1('promptpile-react'), packageRootV1('promptpile-mcp'), packageRootV1('@rustmcp/rust-mcp-filesystem'),
  ]);

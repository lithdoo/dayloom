...
export interface ToolArtifactPolicyV1 { allowedToolNames: readonly string[]; maxToolCallsPerThought: number; maxToolResultLineBytes: number }
export interface ValidatedToolCallV1 { id: string; name: string; arguments: string; raw: Readonly<Record<string, unknown>> }
const CALL_SUFFIX = '.calls.jsonl';
const RESULT_SUFFIX = '.result.jsonl';
export const SYNTHETIC_TOOL_ERROR_V1 = '[DAYLOOM_ASSISTANT_TOOL_ERROR]\nTool execution failed. Treat the requested read or write as unresolved.';
...
export function writeToolResultsV1(calls: readonly ValidatedToolCallV1[], resultPath: string, contents: readonly string[], policy: ToolArtifactPolicyV1): void {
  ...
  writeRowsAtomicV1(resultPath, calls.map((call, index) => boundedRowV1({ tool_call_id: call.id, name: call.name, content: contents[index]! }, policy.maxToolResultLineBytes)));
}
...
export function sanitizeToolResultsV1(calls: readonly ValidatedToolCallV1[], resultPath: string, policy: ToolArtifactPolicyV1): void {
  ...
  writeRowsAtomicV1(resultPath, rows.map((raw, index) => {
...
function recordsV1(file: string): Record<string, unknown>[] {
  ...
  return lines.map((line) => {
    ...
    if (typeof value !== 'object' || value === null || Array.isArray(value)) throw new Error('Tool Artifact line is not an object.');
...
function boundedRowV1(raw: Record<string, unknown>, maxBytes: number): Record<string, unknown> {
  ...
  raw.content = `${typeof raw.content === 'string' ? raw.content.slice(0, 256) : ''}${suffix}`;
  if (Buffer.byteLength(JSON.stringify(raw), 'utf8') > maxBytes) raw.content = suffix;
  return raw;
}

...
import {
  SYNTHETIC_TOOL_ERROR_V1, assertToolCallPolicyV1, pairedResultPathV1,
  readPromptpileToolCallsV1, sanitizeToolResultsV1, writeSyntheticToolResultsV1,
  writeToolResultsV1, type ToolArtifactPolicyV1,
} from './artifacts.js';
...
export interface FileHookConfigV1 extends ToolArtifactPolicyV1 {
  version: 1; promptpileMcpBin: string; baseUrl: string; token: string; execRequestTimeoutMs: number;
  worldRoot: string | null; draft: DraftHookAuthorityV1 | null;
}
export function readFileHookConfigV1(configPath: string): FileHookConfigV1 {
  ...
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('File hook config must be an object.');
  const config = value as FileHookConfigV1;
  if (
    config.version !== 1 || typeof config.promptpileMcpBin !== 'string' || !path.isAbsolute(config.promptpileMcpBin)
    ...
    || !(config.draft === null || validDraftV1(config.draft))
  ...
  ) throw new Error('File hook config is invalid.');
  return config;
}
export async function runFileHookV1(configPath: string): Promise<void> {
  ...
  try { assertToolCallPolicyV1(calls, config); assertAuthorityPolicyV1(calls, config); }
  catch { writeSyntheticToolResultsV1(calls, resultPath, SYNTHETIC_TOOL_ERROR_V1, config); return; }

  const deletes = calls.filter((call) => call.name === 'mcp__draft__delete_file');
  if (deletes.length > 0) {
    if (config.draft === null || deletes.length !== calls.length) { writeSyntheticToolResultsV1(calls, resultPath, SYNTHETIC_TOOL_ERROR_V1, config); return; }
    const draft = config.draft;
    writeToolResultsV1(calls, resultPath, calls.map((call) => {
      try {
        ...
        unlinkSync(canonicalizeExistingPrefixV1(resolveRelativeInsideV1(draft.mcpRoot, requested.path)));
        return `[DAYLOOM_DELETE_FILE_OK]\nDeleted ${requested.path}.`;
      } catch (error) { return `${SYNTHETIC_TOOL_ERROR_V1}\n${error instanceof Error ? error.message : 'delete_file failed.'}`; }
    }), config);
    return;
  }
  try {
    const result = await runNodeCliV1(config.promptpileMcpBin, [
      'exec-calls', '--base-url', config.baseUrl, '--token', config.token,
      '--input', callsPath, '--timeout-ms', String(config.execRequestTimeoutMs), '--overwrite-results',
    ], { timeoutMs: config.execRequestTimeoutMs + 2_000 });
    if (result.code === 0) { sanitizeToolResultsV1(calls, resultPath, config); return; }
  } catch { /* converted to explicit tool evidence */ }
...
function validDraftV1(value: DraftHookAuthorityV1): boolean {
  if (value.mode === 'files') return path.isAbsolute(value.mcpRoot) && Array.isArray(value.files) && value.files.length > 0 && value.files.every(path.isAbsolute);
  return value.mode === 'directory' && path.isAbsolute(value.mcpRoot) && path.isAbsolute(value.root);
}
const isMain = process.argv[1] !== undefined && path.resolve(process.argv[1]) === path.resolve(fileURLToPath(import.meta.url));
if (isMain) {
  const configPath = process.argv[2];
  if (!configPath) { process.stderr.write('File hook config path is required.\n'); process.exitCode = 1; }
  else runFileHookV1(configPath).catch((error) => { process.stderr.write(`${error instanceof Error ? error.message : 'File hook failed.'}\n`); process.exitCode = 1; });
}

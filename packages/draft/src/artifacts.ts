...
export function readPromptpileToolCallsV1(callsPath: string): ValidatedToolCallV1[] {
  const calls = recordsV1(callsPath).map((raw) => {
    ...
    if (!parsed || parsed.id === '' || parsed.type !== 'function' || parsed.function.name === '') {
      throw new Error('ToolCall artifact violates Promptpile ToolCall V1.');
    }
...
export function assertToolCallPolicyV1(calls: readonly ValidatedToolCallV1[], policy: ToolArtifactPolicyV1): void {
  if (calls.length > policy.maxToolCallsPerThought) {
    throw new Error(`Thought emitted more than ${policy.maxToolCallsPerThought} ToolCalls.`);
  }
...
export function writeToolResultsV1(
  calls: readonly ValidatedToolCallV1[],
  resultPath: string,
  contents: readonly string[],
  policy: ToolArtifactPolicyV1,
): void {
...
export function writeSyntheticToolResultsV1(
  calls: readonly ValidatedToolCallV1[],
  resultPath: string,
  content: string,
  policy: ToolArtifactPolicyV1,
): void {
  writeRowsAtomicV1(resultPath, calls.map((call) => boundedRowV1({
    tool_call_id: call.id,
    name: call.name,
    content,
  }, policy.maxToolResultLineBytes)));
...
export function sanitizeToolResultsV1(
  calls: readonly ValidatedToolCallV1[],
  resultPath: string,
  policy: ToolArtifactPolicyV1,
): void {
  ...
  const bounded = rows.map((raw, index) => {
    ...
    if (!parsed || parsed.tool_call_id !== call.id || (parsed.name !== undefined && parsed.name !== call.name)) {
      throw new Error('ToolResult vector is malformed or out of order.');
    }
...
export const SYNTHETIC_TOOL_ERROR_V1 = TOOL_ERROR;
...
function recordsV1(file: string): Record<string, unknown>[] {
  ...
  if (lines.length === 0 || lines.some((line) => line.trim() === '')) {
    throw new Error(`Tool Artifact is empty or malformed: ${path.basename(file)}.`);
  }
  return lines.map((line, index) => {
    const value: unknown = JSON.parse(line);
    if (typeof value !== 'object' || value === null || Array.isArray(value)) {
      throw new Error(`Tool Artifact line ${index + 1} is not an object.`);
    }
...
function boundedRowV1(raw: Record<string, unknown>, maxBytes: number): Record<string, unknown> {
  ...
  const truncation = '\n[DAYLOOM_TOOL_RESULT_TRUNCATED]\nResult exceeded the Dayloom tool-result limit.';
  const content = typeof raw.content === 'string' ? raw.content : '';
  raw.content = `${content.slice(0, 256)}${truncation}`;

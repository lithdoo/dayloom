...
test('Profile V1 typed Revise applies semantic operations while preserving history namespaces', async (t) => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'core2-rich-revise-')), config = path.join(root, 'llm.toml'); fs.writeFileSync(config, '[[llm_api]]\nname="test"\nmodel="test"\n'); t.after(() => fs.rmSync(root, { recursive: true, force: true }));
...
test('Profile V1 typed Revise rejects dangling references without publishing', async (t) => {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'core2-bad-revise-')), config = path.join(root, 'llm.toml'); fs.writeFileSync(config, '[[llm_api]]\nname="test"\nmodel="test"\n'); t.after(() => fs.rmSync(root, { recursive: true, force: true }));

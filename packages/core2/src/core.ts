...
interface InternalOptions { runner?: ProcessRunner; boundaries?: PackagedBoundaries }
...
class DayloomCoreImpl implements DayloomCore {
  ...
  constructor(private world: PublishedWorld, private readonly worldRoot: string, private readonly runtimeRoot: string, private readonly config: CallerConfig, private readonly boundaries: PackagedBoundaries, private readonly runner: ProcessRunner) {}
  getState() { return buildState(this.world.view, this.sessionStatus, this.mutation); }
  ...
  submit() { return this.operation(async () => {
    ...
    try {
      const final = await runReact({ runner: this.runner, reactBin: this.boundaries.reactBin, validate: this.boundaries.validateAgentEvent, config: session.submitConfig, context: session.contextDir, conversation: session.conversationDir, onChild: (child) => { this.activeChild = child; } });
      const submission = parsePlaySubmissionV1(final), documents = validateAndBuildPlayDocuments(session.pinned.playContext!.plan, submission);
      if (this.disposed) return failure('DISPOSED', 'Core is disposed.');
      this.world = await publishPlay(this.worldRoot, session.pinned, session.day, documents);
      this.activeChild = null; this.session = null; this.sessionStatus = null; return success();
    } catch (error) {
      ...
      if (code === 'WORLD_CONFLICT') return failure('WORLD_CONFLICT', (error as Error).message);
      return failure(code === 'SUBMISSION_INVALID' ? 'SUBMISSION_INVALID' : error instanceof SyntaxError || error instanceof Error && /Submission|PlaySubmission/.test(error.message) ? 'SUBMISSION_INVALID' : 'AGENT_FAILED', error instanceof Error ? error.message : 'Submission failed.');
    }
  }); }
...
export async function createDayloomCoreInternal(options: CreateDayloomCoreOptions, internal: InternalOptions = {}): Promise<DayloomCore> {
  ...
  try {
    ...
    return new DayloomCoreImpl(world, worldRoot, runtimeRoot, config, boundaries, internal.runner ?? nodeProcessRunner);

...
export async function createSessionWorkspace(runtimeRoot: string, id: string, config: CallerConfig, definition: WorkspaceDefinition): Promise<CoreSession> {
  ...
  await Promise.all([writeFile(thought, definition.thought), writeFile(sendFinal, definition.sendFinal), writeFile(submitFinal, definition.submitFinal), writeFile(summaryPromptPath, SUMMARY_SYSTEM_PROMPT)]);

...
export async function createSessionWorkspace(runtimeRoot: string, id: string, config: CallerConfig, definition: WorkspaceDefinition): Promise<CoreSession> {
  ...
  const thought = path.join(react, 'thought.md'), observe = path.join(react, 'observe.md'), sendFinal = path.join(react, 'final-send.md'), submitFinal = path.join(react, 'final-submit.md'), sendConfig = path.join(react, 'send.toml'), submitConfig = path.join(react, 'submit.toml');
  const summaryPromptPath = path.join(compression, 'summary.system.md'), summaryConfigPath = path.join(compression, 'summary.toml');
  await Promise.all([writeFile(thought, definition.thought), writeFile(observe, DAYLOOM_OBSERVE_PROMPT), writeFile(sendFinal, definition.sendFinal), writeFile(submitFinal, definition.submitFinal), writeFile(summaryPromptPath, SUMMARY_SYSTEM_PROMPT)]);
  await writeDerivedConfigs(config, { thought, observe, sendFinal, submitFinal, sendConfig, submitConfig, summaryConfig: summaryConfigPath });

...
export function parseDomainPatchV1(value: unknown): DomainPatchV1 {
  ...
  if (value.op === 'move-character' && exact(value, ['op', 'characterId', 'expectedLocationId', 'locationId']) && nonempty(value.characterId) && nullable(value.expectedLocationId) && nullable(value.locationId)) return { op: value.op, characterId: value.characterId, expectedLocationId: value.expectedLocationId, locationId: value.locationId };
...
function nullable(value: unknown): value is string | null { return value === null || nonempty(value); }
function scalar(value: unknown): value is ScalarV1 { return value === null || ['string', 'number', 'boolean'].includes(typeof value) && !(typeof value === 'number' && !Number.isFinite(value)); }

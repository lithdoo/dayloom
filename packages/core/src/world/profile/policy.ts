...
export type DayloomMutationType = 'init' | 'planning' | 'play' | 'revise' | 'settle' | 'abandon-day' | 'migration';

const BUSINESS_NAMESPACE = /^(?:profile|canon|state|characters|locations|arcs|memory|story-seeds|days|audit|custom|legacy)\//;
...
export function expectedMediaTypeV1(documentPath: string): ArchiveMediaTypeV1 {
  ...
  if (path.startsWith('legacy/') && path.endsWith('.preserved.txt')) return 'text/plain';
...
export function assertMutationPathAllowedV1(operation: DayloomMutationType, rawPath: string): string {
  ...
  if (operation !== 'migration' && documentPath.startsWith('legacy/')) throw denied(operation, documentPath);
  if (operation !== 'init' && operation !== 'migration' && documentPath.startsWith('profile/')) throw denied(operation, documentPath);
  ...
  if (operation === 'settle' && (documentPath.startsWith('profile/') || documentPath.startsWith('canon/') || documentPath.startsWith('audit/') || documentPath.startsWith('legacy/'))) throw denied(operation, documentPath);
  if (operation === 'revise' && (/^days\//.test(documentPath) || documentPath.startsWith('legacy/'))) throw denied(operation, documentPath);
  return documentPath;
}

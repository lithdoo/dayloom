...
export const CORE_SEMANTIC_SUMMARY_PROVIDER_ID = 'dayloom-core-semantic-summary-v1';

export const CORE_COMPRESSION_POLICY = Object.freeze({
  threshold: 32_000,
  keepRecent: 4,
...
export interface CoreSemanticSummaryProviderHandle {
...
export function createCoreSemanticSummaryProvider(options: ProviderOptions): CoreSemanticSummaryProviderHandle {
  let inFlight: Promise<unknown> | null = null;
  const provider: SemanticSummaryProvider = {
    id: CORE_SEMANTIC_SUMMARY_PROVIDER_ID,
...
export async function runCompressedCompletion<T>(options: RunCompressedCompletionOptions<T>): Promise<T> {
  const providerHandle = createCoreSemanticSummaryProvider(options);
  ...
  try {
    result = await runCompressionBeforeCompletion({
      compression: {
        ...
        summary: { ...CORE_COMPRESSION_POLICY.summary, provider: providerHandle.provider },
      },
      completion: async () => {
  ...
  if (lifecycleError?.code === 'INVALID_OPTIONS') {
    throw new CoreOperationError('INTERNAL_ERROR', 'Core compression policy is invalid.');
  }

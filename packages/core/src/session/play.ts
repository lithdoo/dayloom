...
import { PLAY_SEND_FINAL_PROMPT, PLAY_THOUGHT_PROMPT } from './prompts/play';
...
export async function createPlayWorkspace(runtimeRoot: string, id: string, world: PublishedWorld, config: CallerConfig, tooling?: SessionToolingBinding): Promise<PlaySession> {
  ...
  return createSessionWorkspace(runtimeRoot, id, config, { kind: 'play', thought: PLAY_THOUGHT_PROMPT, sendFinal: PLAY_SEND_FINAL_PROMPT, pinned: world, day: world.commit.control.day }, tooling) as Promise<PlaySession>;
}

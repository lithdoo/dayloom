...
export function createArchiveV2RuntimeOperations(options:{archive:ArchiveV2Repository}):RuntimeOperations {
  ...
  return {
    ...
    async executeStableCommand(request){const operation=await options.archive.beginWorldOperation({type:request.command,operationId:request.operationId});await options.archive.prepare(operation.id,{phase:publishedPhase(request.target.phase),day:request.target.day,lastSettledDay:request.target.lastSettledDay});return snapshot(request.previous,await options.archive.publish(operation.id));},
  };
}

...
test('visible transcript restores compressed turns and excludes summary and hidden agent Final', async (t) => {
  ...
  fs.writeFileSync(path.join(root, '[4]user.md'), 'third question');
  fs.writeFileSync(path.join(root, '[5]assistant.md'), 'hidden agent final');
  fs.writeFileSync(path.join(root, '[5]assistant.calls.jsonl'), '{"private":true}\n');
  const transcript = await exportVisibleTranscriptV1(root, 'hidden agent final');
  assert.deepEqual(transcript.turns, [
    ...
    { index: 4, role: 'user', content: 'third question' },
  ]);

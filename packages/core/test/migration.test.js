...
test('legacy filesystem migration inventories every file once and completes the restart lifecycle', async (t) => {
  const parent = fs.mkdtempSync(path.join(os.tmpdir(), 'core-migration-')), source = path.join(parent, 'legacy'), target = path.join(parent, 'archive'); t.after(() => fs.rmSync(parent, { recursive: true, force: true })); fs.mkdirSync(source);
...
test('legacy migration rejects symlinks and overlapping source/target roots', async (t) => {
  const parent = fs.mkdtempSync(path.join(os.tmpdir(), 'core-migration-guard-')), source = path.join(parent, 'source'); fs.mkdirSync(source); write(source, 'manifest.yaml', 'id: x\n'); t.after(() => fs.rmSync(parent, { recursive: true, force: true }));

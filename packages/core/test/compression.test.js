...
const {
  ...
  CORE_SEMANTIC_SUMMARY_PROVIDER_ID,
  createCoreSemanticSummaryProvider,
  runCompressedCompletion,
} = require('../dist/promptpile/compression');

function temporaryRoot(t) {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'core-compression-'));
...
test('semantic provider uses exact Promptpile argv, keeps JSON private, and cleans its request directory', async (t) => {
  ...
  const handle = createCoreSemanticSummaryProvider(providerOptions(root, runner, events));
  assert.equal(handle.provider.id, CORE_SEMANTIC_SUMMARY_PROVIDER_ID);
...
test('semantic provider rejects empty, malformed, and non-object JSON and always cleans up', async (t) => {
  for (const stdout of ['', '{bad', '[]', 'null', '"text"']) {
    ...
    const handle = createCoreSemanticSummaryProvider(providerOptions(root, runner));
...
test('fixed policy uses the frozen growth trigger and public heuristic tokenizer', () => {
  assert.equal(CORE_COMPRESSION_POLICY.threshold, 32_000);
  assert.equal(CORE_COMPRESSION_POLICY.keepRecent, 4);
  assert.equal(CORE_COMPRESSION_POLICY.strategy, 'sliding-window');
  assert.equal(CORE_COMPRESSION_POLICY.tokenizer.kind, 'heuristic-fallback');
  assert.deepEqual(CORE_COMPRESSION_POLICY.summary, { kind: 'semantic', maxOutputTokens: 2_048, timeoutMs: 60_000 });
  assert.equal('modelContextTokens' in CORE_COMPRESSION_POLICY, false);
});
...
test('triggered compact lifecycle restores a below-threshold original without calling the Core provider', async (t) => {
  ...
  let providerCalls = 0;
  const runner = { async run() { providerCalls += 1; throw new Error('Core semantic provider must not run'); } };
...
test('triggered compact lifecycle may restore to no turns without calling the Core provider', async (t) => {
  const root = temporaryRoot(t), conversationDir = path.join(root, 'conversation'), seeded = await seedCompactConversation(conversationDir);

...
test('core-running-session-exposes-cancel-capability', async (t) => {
...
test('core-running-cancel-linearizes-once', async (t) => {
...
test('core-running-cancel-intent-wins-send-completion-race', async (t) => {
...
test('core-running-cancel-kills-active-child', async (t) => {
  const { core, controlled, sending } = await startBlockedSend(t);
  const cancelling = core.cancel();
  assert.equal(controlled.react.child.killed, true);
...
test('core-running-cancel-kills-child-started-after-intent', async (t) => {
  let sendAppend;
  const runner = {
    async run(_bin, args, options = {}) {
...
test('core-running-cancel-stops-future-output-delta', async (t) => {
  const { core, controlled, sending } = await startBlockedSend(t, { finishOnKill: false });
  const deltas = [];
...
test('core-interrupted-send-returns-cancelled', async (t) => {
  const { core, sending } = await startBlockedSend(t);
  const cancelling = core.cancel();
...
test('core-interrupted-send-never-restores-ready', async (t) => {
  const { core, sending } = await startBlockedSend(t);
  const seen = [];
...
test('core-interrupted-send-does-not-terminalize-independently', async (t) => {
...
test('core-running-cancel-leaves-world-unchanged', async (t) => {
...
test('core-running-cancel-terminalizes-session-once', async (t) => {
...
test('core-running-cancel-awaits-provider-drain', async (t) => {
...
test('core-running-cancel-cleanup-failure-does-not-resurrect-session', async (t) => {
...
test('core-repeated-running-cancel-joins-one-terminal-intent', async (t) => {
  const { core, sending } = await startBlockedSend(t);
...
test('core-interrupt-state-clears-after-terminal-outcome', async (t) => {
  const { core, sending } = await startBlockedSend(t);
  const cancelling = core.cancel(); await sending; await cancelling;
...
test('core-new-session-is-not-affected-by-old-cancel-state', async (t) => {
  const { core, sending } = await startBlockedSend(t);
  const cancelling = core.cancel(); await sending; await cancelling;
  assert.deepEqual(await core.startSession('play'), { ok: true });
...
test('core-submitting-cancel-remains-unavailable', async (t) => {
...
test('core-dispose-awaits-pending-interrupt-cancel', async (t) => {
  const { core, controlled, sending } = await startBlockedSend(t, { finishOnKill: false });

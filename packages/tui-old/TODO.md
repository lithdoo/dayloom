...
`@dayloom/tui-old` 是 dayloom 的**游戏主入口**（与 `@dayloom/cli` 并列）：
...
| | `@dayloom/cli` | `@dayloom/tui-old` |
|--|----------------|----------------|
| bin | `dayloom` | `dayloom-tui-old` |
...
```
dayloom-tui-old <world-dir> [options]
...
- [x] 自动 smoke：真实 PTY 启动 `dayloom-tui-old <tmp-world> --no-auto-start` 并执行 `/status`
...
| **真实 PTY E2E** | `dayloom-tui-old <tmp-world> --no-auto-start`，自动聚焦 Textarea 后输入 `/status`，Confirm 直接 y/n，Ctrl+C 退出 |
| **不测** | 像素级渲染、真实 AI |
...
- [x] `npm run build` / `npm test` 在 `@dayloom/tui-old` 通过（含 PTY / SessionIO / throttle / stderr）
...
- `@dayloom/tui-old@0.1.0-beta.5`
- examples：`examples/dayloom-tui-old/`（`run-tui.bat/sh`、`run-quick.*`）仍指向 `dayloom-tui-old`，可继续作 smoke 入口
...
| `packages/core-old/src/shell/index.ts` | `runGameShell` |
| `packages/core-old/src/session-io/types.ts` | 契约 |
| `packages/core-old/src/next/inspect-header.ts` | Header 数据 |

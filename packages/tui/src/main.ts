...
import { parseArgv, resolveLlmConfigPath, usage } from './argv.js';
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const llmConfigPath = path.resolve(resolveLlmConfigPath(parsed));
    ...
    try {
      ...
      const vm = createViewModel(driver, { onExitRequest: exitAfterShutdown });
      mounted = mountApp(vm, { onExitRequest: exitAfterShutdown, diagnostic });
    } catch (error) {

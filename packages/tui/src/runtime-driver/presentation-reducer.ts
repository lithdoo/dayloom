...
export function reducePresentation(state:PresentationState,event:Exclude<CoreEvent,{type:'state.changed'}>):PresentationState{
 ...
 if(event.type==='operation.finished'){const status=event.disposition==='committed'?'completed':event.disposition==='cancelled'?'cancelled':event.disposition==='discarded'?'completed':'failed';let next=replace(state,index,{...work,status,pathStatus:'expired',phase:null,text:'',detail:event.message??event.disposition});if(kind==='response'){const messageIndex=next.items.findIndex((item)=>!isWorking(item)&&item.operationId===event.operationId);if(messageIndex>=0){const message=next.items[messageIndex] as TuiMessage,nextStatus=event.disposition==='committed'?'accepted':event.disposition==='superseded'?'superseded':event.disposition==='discarded'?'discarded':event.disposition==='cancelled'?'cancelled':'error';next=replace(next,messageIndex,{...message,status:nextStatus});}}return next;}
 return state;
}

...
import { runGameShell } from '@dayloom/core';
...
test('TuiSessionIO does not intercept /revise; shell recovers without stderr', async () => {
  const worldDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dayloom-tui-phase-d-'));
...
test('SessionIO write/warn/error never touch process.stderr', () => {
  const worldDir = fs.mkdtempSync(path.join(os.tmpdir(), 'dayloom-tui-stderr-'));
  const stderrChunks = [];
  const originalWrite = process.stderr.write.bind(process.stderr);
  process.stderr.write = ((chunk, encoding, cb) => {
    stderrChunks.push(String(chunk));

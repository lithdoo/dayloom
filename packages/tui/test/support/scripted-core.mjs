export class ScriptedDayloomCore {
  ...
  delta(sessionId, text) { this.emit({ type: 'output.delta', sessionId, text }); }
  ...
  async send(text) {
    ...
    if (this.handlers.send) return this.handlers.send(this, text);
    ...
    if (script.failure) {
      ...
      return failure(script.failure.code, script.failure.message);
    }
    if (this.session?.id !== id) return failure('CANCELLED', 'cancelled');
    this.session = { id, kind, status: 'ready' }; this.changed(); return success();
  }
  async submit() {

...
export type SettleInteractiveOptions = SettlementOptions & { io: SessionIO };
...
export async function runSettleInteractive(
  dir: string,
  options: SettleInteractiveOptions,
): Promise<SessionExit<SettlementResult & { proposalPath?: string }>> {
  const { io, ...settleOptions } = options;
  const t = settleOptions.t ?? createTranslator();
  ...
  const serviceRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'dayloom-settle-service-'));
  ...
  try {
    await io.withLoading('正在准备结算上下文...', async loading => {
      buildSettlementPlayerContext(worldRoot, day, contextRoot);
      loading.update('正在启动只读服务...');
      gateway = await connectOrStartGateway(serviceRoot, contextRoot, settleOptions.mcpBaseUrl, settleOptions.mcpToken);
    ...
    const reply = await io.withLoading('正在生成结算提案...', () =>
      callSettlementAi(buildSettlementPromptInput(worldRoot, day), toolsFile, gateway!.baseUrl, gateway!.token, settleOptions.maxToolRounds ?? 8, settleOptions.keepSession, io));
    ...
    if (settleOptions.dryRun) {
      return { kind: 'completed', result: { worldRoot, day, nextDay, description, applied: false } };
    }
    ...
    applySettlement(worldRoot, proposal, changes, nextDay);
    return { kind: 'completed', result: { worldRoot, day, nextDay, description, applied: true } };
  } finally {
    if (gateway) await gateway.stop();
    if (settleOptions.keepSession) io.warn(`Settlement service session preserved at: ${serviceRoot}\n`);
    else fs.rmSync(serviceRoot, { recursive: true, force: true });
  }
}

/** @deprecated Use runSettleInteractive */
export const settleWithAi = runSettleInteractive;

...
| 组件 | 位置 | 用途 |
|------|------|------|
| `promptpile` | `promptpile` | 调用模型，维护消息目录 |
| `promptpile-mcp` | `promptpile-mcp` | 启动 MCP 网关、导出工具定义、执行 MCP 调用 |
| `promptpile-react` | `promptpile-react` | 驱动 AI 多轮探索：Thought -> Observe -> Check -> Final |
...
```bash
dayloom revise -d ./world-interactive
```

建议支持：

```bash
dayloom revise -d ./world-interactive --dry-run
dayloom revise -d ./world-interactive --yes
dayloom revise -d ./world-interactive --keep-session
dayloom revise -d ./world-interactive --mcp-base-url http://127.0.0.1:8765
```
...
```bash
dayloom revise -d <world> --dry-run
```
...
1. 新增 `dayloom revise rollback <revision_id>`。
2. 增加更细的状态变更操作。
...
```text
dayloom revise -d <world> --proposal <fixture.json> [--dry-run] [--yes]
```

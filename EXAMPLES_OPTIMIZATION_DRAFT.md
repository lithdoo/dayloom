...
- 当前 Archive V2 + `@dayloom/core2` + `@dayloom/tui`。

这会产生一个直接问题：用户进入 `examples/` 后无法仅凭目录判断哪一套仍是当前产品路径。旧示例中的 `init`、`daily`、`revise`、`settle`、YAML World、`.loom/`、MCP wiring 与当前 Core2 application API 并不属于同一个运行模型，但仍以一等示例存在。
...
2. 不把 legacy `init` / `daily` / `revise` / `settle` 迁移成 Core2 示例。
3. 保留一个最小、真实、当前可执行的 TUI 示例。
4. 示例中的 provider/model/credential policy 与正式 TUI/Core2 contract 一致。
5. 示例不得重新引入 Core2 已禁止的 MCP、provider override、旧 World layout 或兼容层。
...
- 为 Core2 增加 `init`、`daily`、`revise`、`settle` API；
...
- 修改 Core2、TUI 或 Archive Protocol 的 application boundary。
...
- 与 Core2 当前唯一 `play` Session application surface 不一致。
...
- 名称中的 `play` 与 Core2 Play 同名，但 runtime/data contract 完全不同，继续保留反而更易误导。

不原地改造成 Core2 示例。若未来需要 direct Core2 API example，应新建一个语义明确的独立目录。
...
+ llm.toml
→ build archive-protocol/core2/tui
...
1. 这是当前 `@dayloom/tui` + `@dayloom/core2` 示例。
...
- `[promptpile]` 不包含 Core2 已禁止 caller 拥有的 Conversation/runtime topology 字段；
...
```text
archive-protocol → core2 → tui build
...
- build `@dayloom/archive-protocol`、`@dayloom/core2`、`@dayloom/tui`；
- 只启动 current TUI；
- 不写 World。
...
- 不含 `[promptpile-react]`；
- 不含 Core2-owned topology/runtime fields；
...
- Examples 不复制 Core2/TUI business legality。
...
6. `open-world.*` 可以明确启动当前 Core2 TUI。
...
Core2 owns Play application semantics.

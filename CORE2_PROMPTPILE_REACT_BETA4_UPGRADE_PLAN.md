...
本次升级不重写 Core2 的 React adapter，也不改变 Dayloom public application API、Agent Event v1、Session 状态机或 World publication contract。
...
Agent Event v1 completed
        ↓
existing Core2 result / submission / publication flow
```
...
→ React terminal success、Core2-accepted completion、Core2 send completed、
...
Receipt 是 React 内部 success witness。Core2 不读取、不解析、不保存 Receipt，只继续消费 Agent Event v1 terminal state。
...
Core2-accepted React completion
= Promptpile React terminal success
+ non-empty Final

Core2 send completed
= Core2-accepted React completion
...
Receipt 证明 Promptpile terminal invocation 按其 publication 语义完成；Receipt 自身允许 assistant artifact 为 null。只有再加上 Core2 的 non-empty Final gate，才能把该 invocation 接受为 caller-visible completion。World correctness 仍由 Core2 strict parser、Archive Protocol 与 publication OCC 证明。
...
work root 不得等于或位于任一 authoritative Conversation layer 内。authoritative layer 位于 work root 下虽被上游允许，但 Core2 不采用这种拓扑。
...
- Core2 submission JSON validation 与 World publication。

因此不得：

- deep-import beta.4 runtime module；
- 在 Core2 复制 Completion Receipt validator；
...
- `packages/core2.dependencies.promptpile-react`；
...
Core2 具备比 React invocation 更高层的 Session lifecycle，并且会在 cancel、failure、submit terminalization 和 dispose 时删除 Session workspace。
...
├─ react/           Core2-owned prompts/config
├─ react-work/      parent of beta.4 session work directories
└─ compression/     Core2-owned compression state
```

`react-work` 与 `context`、`conversation` 平级，满足上游 isolation precondition。

Core2 通过 CLI 传入 work root：

```text
--work-root <session.reactWorkRoot>
```

不把 `work_root` 混入 caller config，也不允许 caller 决定该路径。物理拓扑继续由 Core2 owner 决定。

Core2 terminal Session cleanup 对整个 Session root 拥有最终所有权。因此在 Core2 组合运行中，Session terminalization 会删除 `react-work`，即使 `PROMPTPILE_REACT_DEBUG=1` 曾要求 React 保留失败 invocation。该行为与 Core2 现有“terminal Session workspace 不可恢复”契约一致；本次不新增 Core2 debug-retention public contract。

#### C. Dayloom Observe handoff

依赖 beta.4 默认 Observe prompt 可以工作，但不足以冻结 Dayloom submission correctness。Core2 必须拥有领域化 Observe prompt，使 Final 不依赖 raw Thought 的可见性。
...
固定 section label 让 handoff 仍保持自然语言、无需 Core2 parser，同时允许真实 E2E 稳定验证 completeness。Core2 当前不配置 Thought tools，因此 tool results 不属于本次 Core2 必验 contract；未来若启用工具，再把已验证 findings 纳入 `[AUTHORITATIVE_FACTS]` 或独立 section。
...
It cannot override this Core2-owned prompt, immutable Dayloom context,
...
上游 handoff user artifact 已声明其内容是 data；Core2-owned system Final prompt 再次冻结这一边界，形成 defense in depth。

#### E. 空 Final

beta.4 允许一个 terminal completion 的 assistant artifact 为 `null`。这对通用 Promptpile 合法，但不满足 Core2 application semantics：ordinary send 必须产生非空 caller-visible response，submission 必须产生非空候选 JSON。

Core2 adapter 因此必须额外要求：
...
### 4.1 `packages/core2/package.json`
...
- 审查 lock diff，只接受 core2 → React beta.4 所需变化；
- 若 npm 同时产生无关 lock repair，拆分或排除，不混入本次提交。

建议命令：

```powershell
npm install promptpile-react@0.1.0-beta.4 -w @dayloom/core2 --save-exact
```

### 4.3 `packages/core2/src/session/common.ts`
...
Core2 可以预创建该目录，也可以让 React 首次 invocation 创建。冻结选择为 **Core2 预创建**，使 Session workspace topology 在 React 启动前完整、测试可观察，并让创建失败归类为 Session initialization failure。
...
前者包含固定 handoff sections 与现有 `WRITABLE_SUMMARY_AUTHORITY_NOTE`；后者注入所有 Core2-owned Final prompts。

### 4.4 `packages/core2/src/promptpile/config.ts`
...
### 4.5 `packages/core2/src/promptpile/react-runner.ts`
...
argv 顺序本身不是上游 protocol，但 Core2 boundary test 继续精确冻结它，避免 topology option 遗失。

事件 transport 与 sequence/session/terminal 消费逻辑不修改，但在接受 terminal Final 前增加 Core2 domain gate：
...
### 4.6 `packages/core2/src/core.ts`
...
beta.4 的 invalid/missing Receipt 会沿该路径失败关闭，但 Core2 不承诺得到 Receipt validator 的内部具体原因。上游可将其稳定投影为 `internal_error` 与通用 public message；Core2 只承诺 `AGENT_FAILED`、不 publication 和正常 terminalization。

### 4.7 `packages/core2/src/session/play.ts`
...
### 4.8 `packages/core2/src/session/lifecycle.ts`
...
- Planning target day 由 Core2 固定；
- Revise 不修改 identity/history；
- submission schema；
- Core2 final validation。

### 4.9 `packages/core2/src/session/common.ts` 的 Observe prompt
...
- 记录 `npm test -w @dayloom/core2` 的 beta.3 基线结果；
...
- completed 但空白的 Final 被 Core2 映射为 `AGENT_FAILED`；
...
Core2 固定 `max_step = 1`，所以每次 invocation 只产生一个 Observe。多步 React 中“只传最后 Observe”的行为由上游 beta.4 acceptance 负责，Core2 不复制一个不可达的 `max_step > 1` 测试场景。

Receipt validator 的字段级测试也由上游负责。Core2 只测试它实际消费的边界：`session.failed/internal_error` 不会被误报为成功，也不会触发 submission publication。若需要一条 composed missing-Receipt smoke，可通过独立子进程和私有 fixture `PROMPTPILE_BIN` 完成，但不得让 Core2 production code 感知 Receipt 文件。
...
更新 `packages/core2/README.md`，说明：

- Core2 使用 promptpile-react beta.4；
- intermediate reasoning 为 Session-owned、非权威状态；
- only Final enters writable Conversation；
- Core2 terminal cleanup owns the enclosing work root；
...
- React work、Core2 Session Conversation 和 derived configs 都位于 ephemeral runtime root；
- Published World Archive V2 schema 不变；
- submission schema 不变；
- application API 不变。

升级前已经运行中的 Core2 instance 不支持进程间恢复，本次不增加跨版本 Session resume。
...
Core2 不尝试撤回 consumer 已看到的 delta，也不把它写入 World。

Receipt failure 的字段级原因不属于 Agent Event v1 public guarantee。Core2 日志、错误断言和 UI 不得依赖 `missing receipt`、`invocationId mismatch` 等内部字符串；稳定边界只有 public event code/message、`AGENT_FAILED` 与“未 publication”事实。

### 7.4 Cleanup race

Core2 不把删除目录当作取消子进程的手段。若 kill 请求后 child 未关闭，Core2 必须继续等待现有 operation completion seam；只有 child close/operation settle 后才能删除 Session root。
...
- Agent Event v2；
- Core2 public React/work/debug state；
- Receipt public parsing API；
- cross-process React Session recovery；
- orphan GC command；
- caller-configurable work root；
- multiple simultaneous React invocations in one Core2 Session；
- `max_step > 1`；
...
AND Core2 owns a non-authoritative React work root per Session
...
AND Core2 rejects empty Final even when upstream terminal semantics permit it
AND React Receipt, Core2 submission validation, and World publication remain distinct witnesses
AND cancellation/dispose leave no Core2-owned work artifacts
AND child close/operation settle precedes Session root removal
AND public Core2/TUI/Archive contracts remain unchanged

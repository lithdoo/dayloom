...
export function dayDocumentPath(day: string, name: 'plan' | 'play' | 'summary' | 'settlement' | 'abandoned'): string {
  if (!/^day_[0-9]{4,}$/.test(day)) throw new Error('Invalid Dayloom day id.');
  const extension = name === 'plan' || name === 'play' ? 'json' : 'md';
  return normalizeWorldDocumentPathV1(`days/${day}/${name}.${extension}`);
}
...
export function assertDayloomMutationAllowed(path: string, publishedPaths: ReadonlySet<string>, operationType: string): string {
  ...
  if (/^days\/[^/]+\/(play\.json|summary\.md)$/.test(canonical) && publishedPaths.has(canonical) && operationType !== 'revise') {

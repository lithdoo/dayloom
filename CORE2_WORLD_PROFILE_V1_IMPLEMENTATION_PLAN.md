# Dayloom Core2 World Profile V1 完整存档闭环实施冻结

> 状态：**Implemented / 全链路验收完成**
> 冻结日期：2026-08-22；落地验收：2026-08-23
> 目标包：`@dayloom/core2`
> 数据基础：`@dayloom/archive-protocol` V2
> 迁移来源：早期 filesystem World、现有 Core2 World Profile V0
...
Phase A–G 已全部落地，并通过 Core2 build、architecture guard、单元/集成测试。最终 fixture 已验证：
...
```text
dayloom-core2 archive migrate-world-profile-v1 \
  --source <legacy-world> \
...
严格领域 Submission
        ↓
Core2 semantic validator + deterministic document builder
        ↓
WorldChange[]
        ↓
...
Archive V2 已经拥有路径、media type、blob、tree、commit、operation、OCC 和恢复分类。当前缺口位于 Core2 World Profile V0、Submission 数据容量和 Settle 语义。
...
2. 本文 Core2 World Profile V1
   → World document shape、authority、生命周期 mutation

3. CORE2_FUNCTIONAL_COMPLETION_DRAFT.md 中未被本文替换的 runtime 定理
   → public API、Session、publication、cleanup、error semantics

4. CORE2_PROMPTPILE_REACT_BETA4_UPGRADE_PLAN.md
...
模型只提交领域数据；所有路径、稳定 ID、序号、格式化 bytes 和 `WorldChange[]` 均由 Core2 确定性生成。

### 2.3 Single validator invariant

发布前 candidate tree 与启动后 Published World 必须调用同一套 Profile validator。Core2 能发布的 World 必须能在 dispose/restart 后读取。
...
YAML object 必须拒绝 duplicate keys 和 unknown top-level fields。Core2 增加对 `yaml` 的直接依赖，不得依赖 `archive-protocol` 的传递依赖。
...
每个结构化条目必须拥有 Core2-generated stable ID 和 `sourceEventIds`。无法追溯来源的 Init 初始事实使用空数组，并明确 `origin: init`。
...
V1 event ID 固定为同日 `event1`、`event2`……；ID 由 Core2 按提交顺序生成。
...
只保存 writable Conversation 中的 user 和 visible assistant turns。`submission.json` 保存 Core2 接受的领域 Submission，不保存模型未校验的原始 Final bytes。
...
`key` 只在 Submission 内建立引用；Core2 将其按 canonical array order 映射为 `character1`、`location1`、`arc1`、`trigger1`、`fact1` 等 stable ID。重复 key、悬空引用和 unknown fields 一律拒绝。
...
Core2 生成 day 和 `beatN`，并将 submission-local beat key 引用转换为 ID。`dependsOn` 必须引用更早的 beat，避免 cycle 和 forward ambiguity。
...
`DomainPatchV1` 只允许 Core2 理解的领域目标，不允许文件路径：
...
```text
packages/core2/src/world/profile/
  descriptor.ts
  paths.ts
  media.ts
  yaml.ts
...
packages/core2/src/world/builders/
  ids.ts
  encode.ts
  init.ts
  planning.ts
...
packages/core2/src/session/
  submission-v2.ts
  settlement.ts

packages/core2/src/migration/
  inventory.ts
  legacy-reader.ts
  mappings.ts
  report.ts
...
验收：legacy fixture 中每个 regular file 恰好映射一次；迁移后 Core2 可继续完整生命周期。
...
- dispose 后无 Core2-owned filesystem access。
...
11. `npm run test -w @dayloom/archive-protocol`、`npm run test -w @dayloom/core2`、`npm run test -w @dayloom/tui` 全部通过。
...
达到以上条件后，Core2 才可以声明：Archive V2 不仅保存不可变文件历史，也完整承载并延续早期 filesystem World 的世界语义。
